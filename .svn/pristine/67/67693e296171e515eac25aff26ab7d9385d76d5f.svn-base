...
export default class Forget extends BaseComponent {
    ...
    render() {
        ...
        return (
            <View style={{height: '100%', backgroundColor: '#fff'}}>
                ...
                <Header title="修改密码" onPress={()=>this.props.navigation.goBack()}/>
    ...
    async _sendCode(){
        ...
        if (!phone){
            Alert.alert('修改密码', '请输入你的手机号码')
            return
        }
        if (phone.length !== 11){
            Alert.alert('修改密码', '请输入正确的11位手机号码')
            return
        }
        let regx = /^1\d{10}$/;
        if (!regx.test(phone)){
    ...
    async _forget(){
        ...
        if (!phone_code){
            Alert.alert('修改密码', '请输入你收到的短信验证码')
            return
        }
        if (phone_code.length !== 4){
            Alert.alert('修改密码', '请输入正确的4位验证码')
            return
        }
        if (!password){
            Alert.alert('修改密码', '请输入设置密码')
            return
        }
        if (password.length < 6){
            Alert.alert('修改密码', '设置密码长度须大于6位字符')
            return
        }
        if (re_password !== password){
            Alert.alert('修改密码', '两次输入的密码不一致')
            return
        }
        ...
        try {
            ...
            if (ret.data.retCode === 0){
                Alert.alert('修改密码', '密码修改成功，请用新设置的密码登录！',[
                    {text: '好', onPress: async () => {
                ...
                Alert.alert('修改密码', ret.data.retMsg)
            }
        }catch (e) {
            this.refs.toast.show('网络通讯错误，请检查网络！')
        }finally {
