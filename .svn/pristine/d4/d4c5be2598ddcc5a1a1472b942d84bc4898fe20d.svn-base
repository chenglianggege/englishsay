...
import {
    View, Text, StyleSheet, Image, Platform, TouchableOpacity, Alert, Dimensions
} from 'react-native';
...
export default class Type5 extends Component {
    ...
    render () {
        ...
        const question_info = question.info[info_idx]
        const info_item = question_info.items[item_idx]
        let item_keyword = JSON.parse(info_item.item_keyword)
        //console.log('info_item.img_source_content', info_item.img_source_content)
        return (
            <View style={{flex: 1,backgroundColor:"#fff"}}>
                <View style={{flex: 5}}>
                    ...
                    <View style={{flex: 1, alignItems: 'center', marginTop: 30}}>
                        <Image resizeMode="contain" style={{height: 200, marginTop: 10, width: width}} source={{uri: PAPER_STATIC_HOST + info_item.img_source_content}}  />

                        <Text style={{fontSize: 21,  marginTop: 40,color: "#353535"}}>{info_item.item_content}</Text>

                        <Text style={{fontSize: 15,  marginTop: 15, color: "#858585"}}>[{item_keyword.yb}]</Text>

                        <Text style={{fontSize: 15,  marginTop: 15, color: "#858585"}}>{item_keyword.desc}</Text>
