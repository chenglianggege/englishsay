...
// 显示小题的所有子题，逐子题进行 音频-看题-录音
export default class Type26 extends Component {
    constructor(props) {
        super(props);
                ...
                <ScrollView style={{flex: 5}}>
                    ...
                    {question_info.items.map((item, index)=>{
                        if (item.item_content) {
                            return (<Text key={item.item_id} style={[styles.titleText,{marginTop: 10, marginLeft:15}, index === item_idx && item_count>1 ? {color: "#59ad22"} : {}]}>{item.item_content}</Text>)
                        }
                    })}
                </ScrollView>
