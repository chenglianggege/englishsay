...
export default class TSZXPaper extends BaseComponent {
    constructor(props) {
        super(props);
        this.state = {userInfo: {},listData: [],total: 0, loading:false, page: 1, refreshing: true, loadOnEndReached: true,  netError: false}
        this.loading = false
    }
    async componentDidMount() {
        ...
        this.deEmitter = DeviceEventEmitter.addListener('FinishReload',async ()=>{
            if (this._isMounted){
                console.log('DeviceEventEmitter.addListener.FinishReload')
    ...
    render () {
        const {qsType, qsTypeName} = this.props.navigation.state.params
        const {loading, netError} =  this.state
        return (
            <View style={{flex: 1, backgroundColor: "#fff"}}>
                <Header title={qsTypeName} onPress={()=>this.props.navigation.goBack()}/>
    ...
    async _getPapers (exam_id) {
        try {
            ...
            let listData = await axios.get(global.API_HOST + '/v2/student/exercise/list',{
                params: {paper_type: 3, qs_type: qsType,page: exam_id ? 1 : this.state.page, pagesize: 5, exam_id: exam_id}
            })
